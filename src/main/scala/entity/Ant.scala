...
import utils.Common.{MiusumTimeused_LinkPathInfo, generateOneOrZero, simpleSort}
...
class Ant(val graphInit: GraphInit, val inited_g : Graph[Ant_VertexInfo,Ant_EdgeInfo],
  ...
  private def stepforward(start_id : Long, inputGraph : Graph[Ant_VertexInfo,Ant_EdgeInfo]): (Long, Graph[Ant_VertexInfo,Ant_EdgeInfo]) = {
    ...
    if(useful_edge_num == 0){
      ...
      val dstVertextRDD = useful_edges.filter(et => et.srcId == selectedEdge._1 && et.dstId == selectedEdge._2 )
        ...
        (vd.id, Ant_VertexInfo(vd.id,vd.name,vd.v_type,start_id,/*vd.pher_chosen_Map,*/ vd.backFromId, -1L, vd.pathFromPre)))
      //替换单点属性（更新全图）
  ...
  private def backforward(end_id: VertexId, inputGraph : Graph[Ant_VertexInfo,Ant_EdgeInfo]): (Long, Graph[Ant_VertexInfo, Ant_EdgeInfo]) = {
    ...
    if(useful_edge_num == 0){
      ...
      val dstVertextRDD = useful_edges.map(et => et.dstAttr).map(vd => {
        val preVid = -1L
        (vd.id, Ant_VertexInfo(vd.id,vd.name,vd.v_type,preVid,/*vd.pher_chosen_Map,*/ vd.backFromId, vd.sourceVid, vd.pathFromPre))
      })
      //源节点的backFromId属性设为目的节点的vid
      val srcVertextRDD = useful_edges.map(et => et.srcAttr).map(vd => {
        val backFromId = end_id
        (vd.id, Ant_VertexInfo(vd.id,vd.name,vd.v_type,vd.preVid,/*vd.pher_chosen_Map,*/ vd.backFromId + backFromId,vd.sourceVid, vd.pathFromPre))
      })
      //替换单点属性（更新全图）
      val new_g = inputGraph.joinVertices(dstVertextRDD){
  ...
  private def recordForEndvertex(links: Array[VertexId], inputG : Graph[Ant_VertexInfo,Ant_EdgeInfo]) : Unit = {
    ...
    var new_g = inputG
    for(i <- vidArray.indices){
      if(i + 1 < vidArray.length) {//前置节点可以获得
        val vd: VertexRDD[Ant_VertexInfo] = inputG.vertices.filter(vd => vd._1 == vidArray(i))
        //替换单点属性（更新全图）
        new_g = inputG.joinVertices(vd) {
          (vid, old, newv) => Ant_VertexInfo(newv.id,newv.name,newv.v_type,
            vidArray(i + 1),//设preVid
            newv.backFromId,newv.sourceVid,newv.pathFromPre
          )
        }
      }
    }
    //记录（从源点到目的节点）
    val record_g = new_g.subgraph(epred = et => et.dstAttr.preVid >=0L && et.dstAttr.preVid == et.srcId)
    val endv_graphs = end_vertexes.get(vidArray(0))
    endv_graphs match{
      case Some(x) => x.append(record_g)
      case _ => throw new RuntimeException("结束节点未放入终止节点集合中")
    }
  ...
  private def popFindPath(start_id : Long, inputGraph : Graph[Ant_VertexInfo,Ant_EdgeInfo]): (Long, Graph[Ant_VertexInfo,Ant_EdgeInfo]) = {
    ...
    val edgeinfoInVertex_g: VertexRDD[Map[(VertexId, VertexId), Int]] = inputGraph.aggregateMessages[Map[(VertexId, VertexId), Int]](
      ctx => {
        val srcid = ctx.srcId
        val dstid = ctx.dstId
        val pher = b_pherMuMap.value.getOrElse((srcid,dstid), 0)
        if(pher > 0)
    ...
    //生成新的图
    val popfindgraph: Graph[Ant_VertexInfo,Ant_EdgeInfo] = Graph(inputGraph.vertices, edgeChosenRDD.map(edge => Edge(edge._1._1,edge._1._2,edge._2)))
    val ccvd: VertexRDD[VertexId] = popfindgraph.connectedComponents().vertices.filter(_._2 == start_id)//从0点出发经过的节点id
    //获取所有的和0点关联的节点的vid
    val ccvdColl = ccvd.mapPartitions(_.map(_._1)).collect()
    var endVContained = false
    breakable {
      graphInit.getEnd_vertex.foreach(x => {
        if (ccvdColl.contains(x)) {
          //结尾节点的集合有cc中的节点
          endVContained = true
          break()
        }
    ...
    if(!endVContained)
      popFindPath(start_id, inputGraph) //继续递归
    //继续对ccvd求新图（和inputGraph合并），新图是和sourceVid，即start_id连在一起的所有点形成的图。
    val newg: Graph[Ant_VertexInfo, Ant_EdgeInfo] = Graph(ccvd,popfindgraph.edges).outerJoinVertices(inputGraph.vertices) {
      (vid, zero, input) => {
        input match {
          case Some(v) => Ant_VertexInfo(v.id, v.name, v.v_type,-1L,v.backFromId,start_id,v.pathFromPre)
          case None => Ant_VertexInfo(-1L, "-1L",VertexEnum.unknown,-1L,mutable.Set(),start_id,(Array.empty,-1.0))
        }
      }
    }
    //对新图找到源点0到end节点的链路(不用上面的蚂蚁寻路，用pregel并行)
    //发送的消息结构为(Set(路径vid链条),该链条的总的TC, 当前弧的TC).逻辑就是找TC最小的贪婪
    val pregeledG = newg.pregel(
      ...
      (vid, vd, distMsg) => {
        ...
        if(change) {
          println(s"顶点$vid，属性${vd.pathFromPre}，收到消息$distMsg")
          if(vd.pathFromPre._1.nonEmpty) {//已经有值更新进来过
            if(vd.pathFromPre._2 > distMsg._2) {//保存的链路TC和更大，则更新成消息中的
              println(s"顶点$vid，更新后${(distMsg._1 :+ vid, distMsg._2 + distMsg._3)}")
              if(graphInit.getEnd_vertex.contains(vid))//是结束节点要记录
                recordForEndvertex(distMsg._1 :+ vid, inputGraph)
              Ant_VertexInfo(vd.id,vd.name,vd.v_type,
                distMsg._1.last,
                vd.backFromId,vd.sourceVid,
                (distMsg._1 :+ vid, distMsg._2 + distMsg._3))
            }else {//保留自身的原链路，把自身加入
              println(s"顶点$vid，更新后${(vd.pathFromPre._1 :+ vid, vd.pathFromPre._2 + distMsg._3)}")
              if(graphInit.getEnd_vertex.contains(vid))//是结束节点要记录
                recordForEndvertex(vd.pathFromPre._1 :+ vid, inputGraph)
              Ant_VertexInfo(vd.id,vd.name,vd.v_type,vd.preVid,
                vd.backFromId,vd.sourceVid,
                (vd.pathFromPre._1 :+ vid, vd.pathFromPre._2 + distMsg._3))
            }
          }else{
            //第一次更新pathFromPre
            println(s"顶点$vid，更新后${(distMsg._1 :+ vid, distMsg._2 + distMsg._3)}")
      ...
      edgeTriplet => {
        ...
        if(srcVd.pathFromPre._1.contains(edgeTriplet.dstId)){//会形成环
          println(s"顶点${edgeTriplet.srcId} 给 顶点${edgeTriplet.dstId} 发送消息 ${srcVd.pathFromPre}失败")
          Iterator.empty
        }else{
          println(s"顶点${edgeTriplet.srcId} 给 顶点${edgeTriplet.dstId} 发送消息 ${srcVd.pathFromPre}成功")
          Iterator[(VertexId,(Array[Long], Double, Double))](
            (edgeTriplet.dstId,
